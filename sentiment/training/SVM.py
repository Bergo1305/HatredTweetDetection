...
def construct_count_vectorized_data(
    ...
    model_root_path = f"{CURRENT_DIR}/training/models/SVM"
    pickle.dump(vectorized, open(f"{model_root_path}/vocabulary1.sav", 'wb'))
...
def train() -> None:
    ...
    logs_file = open(f"{model_root_path}/model1.logs", "w")
    ...
    pickle.dump(model, open(f"{model_root_path}/sfm1.sav", 'wb'))
